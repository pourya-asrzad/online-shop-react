...
const ProductionContainer = ({ categoryId }) => {
    ...
    const { data: filterData, isLoading: categoryisLoading = false } = useFetchHomecategoryQuery(categoryId)
    ...
    if (categoryisLoading) {
        ...
        categoryTitleAnswer = <span style={{
            fontSize: "22px",
            fontWeight: 600
        }}>  {filterData[0].name}</span>
    }
    let productAnswer = null
    if (someProductLoading) {
        productAnswer = fakeArray.map((e, index) => {
            return <div key={index} className={Styles.ProductionCardHome}>
    ...
    if (isSuccess) {
        productAnswer = someProducts.map((element) => {
            return <Link key={element.id} className={Styles.link} to={INTERNAL_PATHS.SINGLEPRODUCT + `/${element.id}`}>
    ...
    return (
        <>
            <div className='categorycontainr'>
                ...
                <Link className='link-route' to={filterData ? INTERNAL_PATHS.CATEGORYS + `/${filterData[0].id}-0` : ''}>
